...
def plot_imu(t, d, out):
    ...
    # Motion reference command (19D) — first few channels as overview
    motion_cols = [f"motion_cmd_{i}" for i in range(19) if f"motion_cmd_{i}" in d]
    if motion_cols:
        for col in motion_cols[:6]:
            axes[2, 0].plot(t, d[col], lw=0.7, alpha=0.8, label=col)
    axes[2, 0].set_title("Motion Command (first 6 of 19D)")
    axes[2, 0].legend(fontsize=6, ncol=2); axes[2, 0].grid(True, alpha=0.3)
    axes[2, 0].set_xlabel("time [s]")
    # Value function
    if "value" in d:
        axes[2, 1].plot(t, d["value"], lw=0.8, color="red", alpha=0.7, label="value")
        axes[2, 1].set_ylabel("value", color="red"); axes[2, 1].legend(fontsize=8)
    axes[2, 1].set_title("Value Function"); axes[2, 1].grid(True, alpha=0.3)
...
def plot_vel_tracking(t, d, out):
    """Motion reference command (19D) overview."""
    motion_cols = [f"motion_cmd_{i}" for i in range(19) if f"motion_cmd_{i}" in d]
    if not motion_cols:
        return None
    n = len(motion_cols); ncols = 4; nrows = math.ceil(n / ncols)
    fig, axes = plt.subplots(nrows, ncols, figsize=(20, 3.5 * nrows), sharex=True)
    axes = np.array(axes).reshape(-1)
    fig.suptitle("Motion Reference Command (19D)", fontsize=14)
    for i, col in enumerate(motion_cols):
        axes[i].plot(t, d[col], lw=0.8)
        axes[i].set_title(col); axes[i].grid(True, alpha=0.3); axes[i].set_xlabel("time [s]")
    for i in range(n, len(axes)):
        axes[i].axis("off")
    fig.tight_layout(); path = out / "01b_motion_cmd.png"; fig.savefig(path, dpi=150); plt.close(fig)
    return path
...
def plot_obs_frame(t, d, out):
    obs_cols = [f"obs_{i}" for i in range(63) if f"obs_{i}" in d]
    if not obs_cols:
        return None
    groups = [
        ("ang_vel (obs 0-2)", [0, 1, 2]),
        ("proj_grav (obs 3-5)", [3, 4, 5]),
        ("motion_cmd (obs 6-24)", list(range(6, 25))),
        ("joint_pos_rel (obs 25-37)", list(range(25, 38))),
        ("joint_vel_scaled (obs 38-50)", list(range(38, 51))),
        ("last_action (obs 51-62)", list(range(51, 63))),
    ]
    fig, axes = plt.subplots(len(groups), 1, figsize=(18, 3.5 * len(groups)), sharex=True)
    fig.suptitle("Policy Observation Frame (63D)", fontsize=14)
    ...
    fig.tight_layout(); path = out / "07_obs_frame_63d.png"; fig.savefig(path, dpi=150); plt.close(fig)
    return path
...
def compare_imu(ts, ds, tr, dr, out):
    ...
    # Motion command (first 6 of 19D) — sim vs real
    for i in range(6):
        col = f"motion_cmd_{i}"
        if col in ds:
            axes[2, 0].plot(ts, ds[col], lw=0.7, color=SIM_COLOR, alpha=SIM_ALPHA)
        if col in dr:
            axes[2, 0].plot(tr, dr[col], lw=0.7, color=REAL_COLOR, alpha=REAL_ALPHA)
    axes[2, 0].set_title("Motion Command (first 6 of 19D)")
    ...
    # Value function
    if "value" in ds:
        axes[2, 1].plot(ts, ds["value"], lw=0.7, color=SIM_COLOR, alpha=SIM_ALPHA)
    if "value" in dr:
        axes[2, 1].plot(tr, dr["value"], lw=0.7, color=REAL_COLOR, alpha=REAL_ALPHA)
    axes[2, 1].set_title("Value Function"); _dual_legend(axes[2, 1]); axes[2, 1].grid(True, alpha=0.3)
    axes[2, 1].set_xlabel("time [s]")
...
def compare_obs_frame(ts, ds, tr, dr, out):
    ...
    has_any = any(f"obs_{i}" in ds or f"obs_{i}" in dr for i in range(63))
    if not has_any:
        return None

    fig, axes = plt.subplots(len(groups), 1, figsize=(18, 3.5 * len(groups)), sharex=True)
    fig.suptitle("Policy Obs Frame (63D)  —  sim (blue) vs real (red)", fontsize=14)
    for ax, (label, indices) in zip(axes, groups):
        for idx in indices:
            col = f"obs_{idx}"
            if col in ds:
...
def compare_obs_per_joint(ts, ds, tr, dr, out):
    ...
    # joint_pos_rel (obs 25-37) and joint_vel_scaled (obs 38-50)
    for obs_start, n, tag, title in [
        (25, 13, "joint_pos_rel", "Obs: Joint Pos Rel (obs 25-37)"),
        (38, 13, "joint_vel_scaled", "Obs: Joint Vel Scaled (obs 38-50)"),
        (51, 12, "last_action", "Obs: Last Action (obs 51-62)"),
    ]:
...
def compare_rmse_summary(ts, ds, tr, dr, out):
    ...
    signal_groups = {
        ...
        "q_rel": [f"q_rel_{i}" for i in range(13)],
